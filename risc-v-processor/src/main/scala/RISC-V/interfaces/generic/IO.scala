...
class TrapInterface extends Bundle {
  // Trap Interface
  val trap_valid = Output(Bool())
  val trap_reason = Output(TRAP_REASON())
}
...
class ExecutionUnitInterface extends Bundle {
  ...
  val instr_type = Input(RISCV_TYPE.Type())
}

class RVFIInterface(val rdPorts: Int = 2, val rsPorts: Int = 4) extends Bundle {
  ...
  val rvfi_rs_addr = Output(Vec(rsPorts, UInt(5.W)))
  val rvfi_rs_rdata = Output(Vec(rsPorts, UInt(32.W)))
  val rvfi_rs_rcount = Output(UInt(log2Ceil(rsPorts + 1).W))

  val rvfi_rd_addr = Output(Vec(rdPorts, UInt(5.W)))
  val rvfi_rd_wdata = Output(Vec(rdPorts, UInt(32.W)))
  val rvfi_rd_wcount = Output(UInt(log2Ceil(rdPorts + 1).W))
